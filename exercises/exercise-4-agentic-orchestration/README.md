...
3. Configure:
   ...
   - **Prompt** (FEEL expression):
     ...
     ```
     If AI cannot be used, you can rely on a [DMN table](#alternative-using-dmn-instead-of-ai) to experience the Ad-Hoc Subprocess. 
...
## Alternative: Using DMN Instead of AI

If you don't have access to an AI service (OpenAI API), you can use a **DMN decision table** to determine which tools to execute. A pre-built DMN file `toolChoice.dmn` is included in this module.

### Why Use DMN as Alternative?
...
1. **Open Camunda Modeler** and create a new DMN diagram
2. Add a **Decision** element named "Decide on Tools"
3. Set the Decision ID to `Decision_toolUse`
4. Configure the **Decision Table**:

#### Input Column
| Property | Value |
|----------|-------|
| Label | Customer Message |
| Expression | `customerMessage` |
| Type | `string` |
...
| # | Customer Message (Input) | Tools (Output) |
|---|--------------------------|----------------|
| 1 | `contains(lower-case(customerMessage), "tire")` | `"Tool_4"` |
| 2 | `contains(lower-case(customerMessage), "chain")` | `"Tool_5"` |
| 3 | `contains(lower-case(customerMessage), "light")` | `"Tool_2,Tool_6"` |
| 4 | *(empty - default/catch-all)* | `"Tool_5"` |

> **Note**: The rules use FEEL expressions to match keywords in the customer message. You can extend these rules to cover more scenarios.

### Modifying the BPMN to Use DMN

Replace the **OpenAI Connector** task "Decide on Tool to use" with a **Business Rule Task**:

1. Delete or replace the OpenAI Service Task
2. Add a **Business Rule Task** named "Decide on Tool to use"
3. Configure the task:
    - **Implementation**: DMN Decision
    - **Decision ID**: `Decision_toolUse`
    - **Result Variable**: `toolChoice`
    - **Binding**: `deployment` (to use the deployed DMN)

### Input Mapping for DMN

Ensure the `customerMessage` variable is available in the process scope. The DMN will automatically use it as input.

### Extending the DMN Rules

You can add more sophisticated rules to handle different scenarios:

```
| Input Expression | Output |
|------------------|--------|
| contains(lower-case(customerMessage), "damaged") | "Tool_1,Tool_2,Tool_3" |
| contains(lower-case(customerMessage), "warranty") | "Tool_3,Tool_4" |
| contains(lower-case(customerMessage), "status") | "Tool_1,Tool_4" |
| contains(lower-case(customerMessage), "urgent") | "Tool_1,Tool_5,Tool_6" |
```

### Testing with DMN

Start a process instance with the same test variables:
```json
{
  "customerNo": "CUST-12345",
  "orderId": "ORD-98765",
  "customerMessage": "My bike light is broken and I need a replacement."
}
```

With the DMN rules above, this will match the "light" rule and output `"Tool_2,Tool_6"` (Check Inventory + Create Ticket).
